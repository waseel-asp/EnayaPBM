...
public class PbmconfigPayerToProviderFacade {
	...
	// this method is to get all the Providers who are using PBM
	@SuppressWarnings("unchecked")
	public List<String> findAllProviders() {
		LogUtil.log("finding all List of  Providers PbmconfigPayerToProvider instances", Level.INFO, null);
		try {
			final String queryString = "select model.id.providerId from PbmconfigPayerToProvider model";
			Query query = entityManager.createQuery(queryString);
			return query.getResultList();
		} catch (RuntimeException re) {
			LogUtil.log("find all failed", Level.SEVERE, re);
